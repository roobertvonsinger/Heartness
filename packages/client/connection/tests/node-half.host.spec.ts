...
import { API_PATH, apply, CANVAS_EVENTS_PATH, HOST_EVENTS_PATH, inject, MUX_EVENTS_PATH, VOICE_WS_PATH, type HostConnectionHandle } from '../src/index.ts'
...
describe('connection node half', () => {
  ...
  it('registers one HTTP route plus one upgrade route per downlink and removes all three with the fiber', async () => {
    ...
    expect(upgrades.map(route => route.path)).toEqual([MUX_EVENTS_PATH, HOST_EVENTS_PATH, CANVAS_EVENTS_PATH, VOICE_WS_PATH])
    await dispose()
    expect(routes).toHaveLength(0)
    expect(upgrades).toHaveLength(0)
  })
  ...
  it('requires WebSocket upgrade for network GETs to either event path', async () => {
    ...
    for (const path of [MUX_EVENTS_PATH, HOST_EVENTS_PATH, CANVAS_EVENTS_PATH, VOICE_WS_PATH]) {
