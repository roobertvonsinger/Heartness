...
export class RozRecycleEngine {
  ...
  /** Async-ready initializer; callers should `await engine.init()` before use. */
  async init(): Promise<void> {
    try {
      await mkdir(this.stagingDir, { recursive: true })
    } catch {
      // best effort — staging dir creation is non-fatal
    }
  }

  private async loadManifest(): Promise<Record<string, FileVersionInfo[]>> {
    try {
  ...
  private async saveManifest(manifest: Record<string, FileVersionInfo[]>): Promise<void> {
    try {
  ...
  public async backupFile(filePath: string): Promise<string | undefined> {
    try {
      ...
      if (this.versioningEnabled) {
        await this.createFileVersion(filePath, 'auto-backup')
      }
  ...
  /** Create an immutable versioned snapshot with parent checksum tracking (Async) */
  public async createFileVersion(filePath: string, author = 'antigravity'): Promise<FileVersionInfo | undefined> {
    try {
      ...
      if (latest) {
        try {
          ...
          // parent snapshot unreadable — keep initial diff summary
        }
      }
  ...
  public async listFileVersions(filePath: string): Promise<FileVersionInfo[]> {
    const normalizedPath = resolve(filePath)
    const manifest = await this.loadManifest()
  ...
  public async rollbackFileVersion(filePath: string, versionId: string): Promise<boolean> {
    const versions = await this.listFileVersions(filePath)
    ...
    try {
      ...
      await this.createFileVersion(filePath, `rollback-to-${versionId}`)
      return true
    } catch {
      return false
    }
  ...
  public async backupContextData(contextId: string, data: unknown): Promise<string | undefined> {
    try {
      const today = new Date().toISOString().slice(0, 10)
  ...
  public async restoreContextData(filePath: string): Promise<Record<string, unknown> | undefined> {
    try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            try {
              ...
              // file stat/unlink failure — skip silently during background cleanup
            }
          }
        }
      } catch {
...
export async function registerRozEngine(_ctx: Context, config: RozEngineConfig = {}): Promise<RozRecycleEngine | undefined> {
