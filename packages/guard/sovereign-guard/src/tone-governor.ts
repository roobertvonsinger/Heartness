...
export function registerToneGovernor(ctx: Context, config: ToneGovernorConfig = {}): void {
  ...
  const sysPrompt = ctx.get('systemPrompt') as { section?: (opts: { name: string; order: number; text: string }) => void } | undefined
  ...
  const eventHost = ctx as unknown as { on: (event: string, listener: (response: unknown) => void) => void }
  eventHost.on('agent/response', (response: unknown) => {
